...
class AudioController extends Controller
{
	...
	public function actionCreate()
	{
		...
		$list_data=CHtml::listData($audio_category, 'id', 'name');
        ...
        if(isset($_POST['Audio']))
		{
			...
			{   
                if(!empty($model->image_name)){
                    ...
                    // новое имя для файла
                    $imageName = 'audio_' . time().'.'. $sourcePath['extension'];
                    
                    //Переменной $file присвоить путь, куда сохранится картинка без изменений, параметры тянутся из конфига
                    $file = $_SERVER['DOCUMENT_ROOT'] . Yii::app()->getBaseUrl() . Yii::app()->params['userAudiosPath'] .$imageName;
                    ...
                    ->save($_SERVER['DOCUMENT_ROOT'] . Yii::app()->getBaseUrl() . Yii::app()->params['userAudiosPath'] . $imageName); //перезапись уже ужатой картинкой
