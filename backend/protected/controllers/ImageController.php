...
class ImageController extends Controller
{
	...
	public function actionCreate()
	{
		...
		$list_data=CHtml::ListData($image_category, 'id', 'name');
        ...
        if(isset($_POST['Image']))
		{
			...
			{   
                if(!empty($model->image_name)){
                    ...
                    // новое имя для файла
                    $imageName = 'image_' . time().'.'. $sourcePath['extension'];
                    
                    //Переменной $file присвоить путь, куда сохранится картинка без изменений, параметры тянутся из конфига
                    $file = $_SERVER['DOCUMENT_ROOT'] . Yii::app()->getBaseUrl() . Yii::app()->params['userImagesPath'] .$imageName;
                    ...
                    ->save($_SERVER['DOCUMENT_ROOT'] . Yii::app()->getBaseUrl() . Yii::app()->params['userImagesPath'] . $imageName); //перезапись уже ужатой картинкой
		...
		if(isset($_POST['Image']))
		{
			...
			if($model->validate())
			{
                if(!empty($model->image_name)){
                    ...
                    $sourcePath = pathinfo($model->image_name->getName());
                    // новое имя для файла
                    $imageName = 'about_' . time() . '.' . $sourcePath['extension'];
                    //Переменной $file присвоить путь, куда сохранится картинка без изменений, параметры тянутся из конфига
                    $file = $_SERVER['DOCUMENT_ROOT'] . Yii::app()->getBaseUrl() . Yii::app()->params['userImagesPath'] .$imageName;
                    $model->image_name->saveAs($file);
                    $model->image_name=$imageName;
