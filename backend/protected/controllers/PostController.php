...
class PostController extends Controller
{
	...
	public function actionCreate()
	{
        ...
        // делаем выборку всех категорий из баззы данных
		$postStatusModel = Status::model()->findAll();
		// при помощи listData создаем массив вида $ключ=>$значение
		$listStatus = CHtml::listData($postStatusModel, 'id', 'title');
		...
		if(isset($_POST['Post']))
		{
			...
			{   
                if(!empty($model->image_name)){
                    ...
                    $file = $_SERVER['DOCUMENT_ROOT'].Yii::app()->baseUrl . Yii::app()->params['userPostsPath'] .$imageName;
                    ...
                    ->save($_SERVER['DOCUMENT_ROOT'].Yii::app()->baseUrl . Yii::app()->params['userPostsPath'] . $imageName); //перезапись уже ужатой картинкой
    ...
    public function actionApproved($statId) {
        //выбор всех строк со статусом 1 и сортировка по дате от первых к последним
		$dataProvider=new CActiveDataProvider('Post', array(
            'criteria'=>array(
                'condition'=>'status_id=' . $statId,
            ),
            'sort' => array(
                'defaultOrder' => 'create_time DESC',),
        ));
