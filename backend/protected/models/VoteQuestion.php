...
class VoteQuestion extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('title, status_id', 'required', 'message'=>'Поле {attribute} должно быть заполнено'),
			array('status_id', 'numerical', 'integerOnly'=>true),
			array('title', 'length', 'max'=>255),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, title, status_id', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'voteAnswers' => array(self::HAS_MANY, 'VoteAnswer', 'question_id'),
